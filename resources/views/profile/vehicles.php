...
<div class='col-md-10'>
    ...
    ?>
    <h1>Authorized Vehicles<a onclick='addNewModal()' data-toggle="modal" data-target="#addNewCategory" href='javascript:void()' class="btn btn-primary" style="float:right">Add New</a></h1>

    <table class='table table-striped'>
        <tr><th>Vehicle Picture</th><th>Vehicle Name</th><th>Vehicle No.</th><th>Mileage</th><th>Occupancy</th></th><th>Action</th></tr>
        <?php
        $root=Request::root();
        foreach($results as $key):
            ...
            $edit="<a href='$root/system-admin/vehicle/edit/{$key->vehicle_id}' class='btn btn-info'>Edit</a>";
            $delete="<a href='$root/system-admin/vehicle/{$key->vehicle_id}' class='btn btn-danger'>Delete</a>";
            echo "<tr><td>$pic</td><td>{$key->vehicle_name}</td><td>{$key->vehicle_no}</td><td>{$key->mileage}</td><td>$key->occupancy</td><td>$edit  $delete</td></tr>";
        endforeach;
        ?>
    </table>
</div>
